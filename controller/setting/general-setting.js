...
const deleteStaticPageData = async (req, res) => {
  const id = req.params.id;
  let findStaticPage;
  try{
    findStaticPage = await StaticPage.findById({_id:id});
  }catch(err){
    // console.log(err);
    return res.status(500).json({message:"Something went wrong"});
  }
  if(!findStaticPage)return res.status(404).json({message:"Static page not found"});

  let deletePost;
  try{
    deletePost = await StaticPage.deleteOne({_id:id});
  }catch(err){
    // console.log(err);
    return res.status(500).json({message:"Something went wrong"});
  }
  return res.status(201).json({message:"Deleted post successfully"});
};

const updateStaticPageData = async (req, res) => {
  const id = req.params.id;
  const { heading, description, slugurl, redirecturl, redirect_mode } =
    req.body;
  let pageData;
  try {
  ...
  if (!pageData)
    return res
      .status(404)
      .json({ message: "static page not found by this id" });
...
const staticPageData = async (req, res) => {
  ...
  let createStaticData = new StaticPage({
    heading,
    description,
    slugurl,
    redirecturl,
  ...
  return res
    .status(201)
    .json({ createStaticData: createStaticData, message: "Created" });
};
...
const getSiteData = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Something went wrong while checking document count" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Something went wrong while finding site infromation" });
  }
  if (!data)
    return res
      .status(404)
      .json({ message: "Site information is not available" });
...
const SiteData = async (req, res) => {
  ...
  if (checkAdmin === 0) {
    const createdData = new SiteInformation({
      title,
      m_mode,
      i_mode,
      logo: c_path1,
    ...
    return res
      .status(201)
      .json({ createdData: createdData, message: "Setting data created" });
  } else {
    let findSiteData;
    try {
      ...
      return res
        .status(500)
        .json({ message: "something went wrong while finding site data" });
    }
    if (!findSiteData)
      return res.status(103).json({ message: "Missing site information" });
    ...
    if (findSiteData) {
      ...
      try {
        ...
        return res
          .status(201)
          .json({
            message: "something went wrong while updating site information",
          });
      }
